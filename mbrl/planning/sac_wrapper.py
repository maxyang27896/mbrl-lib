...
import mbrl.third_party.sb3_sac as sb3_sac 
...
class SACAgent(Agent):
    ...
    def act(
        self, obs: np.ndarray, action: np.ndarray = None, sample: bool = False, batched: bool = False, **_kwargs
    ) -> np.ndarray:
...
class SACAgentSb3(Agent):
    ...
    def __init__(self, sac_agent: sb3_sac.SAC):
        self.sac_agent = sac_agent
    ...
    def act(
        ...
        """
        with torch.no_grad():
            return self.sac_agent.sample_action(
                obs, batched=batched, deterministic=not sample
            )
